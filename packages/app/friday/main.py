...
- Current date and time: {current_time}""".format(
    ...
    if os.path.exists(os.path.join(path_dialog_history, FRIDAY_SESSION_ID)):
        await session.load_session_state(friday=agent)
    ...
    # Save dialog history
    await session.save_session_state(friday=agent)
